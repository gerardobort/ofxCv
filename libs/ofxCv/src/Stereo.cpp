...
namespace ofxCv {
    ...
    void Stereo::calibrate(Camera& leftCamera, Camera& rightCamera) {
        double res = stereoCalibrate(
            ...
            CV_CALIB_FIX_INTRINSIC+CV_CALIB_FIX_PRINCIPAL_POINT+CV_CALIB_FIX_FOCAL_LENGTH+CV_CALIB_ZERO_TANGENT_DIST);
        ...
        // Applying Undistort
        initUndistortRectifyMap(leftCamera.cameraMatrixRefined, leftCamera.distortionCoefficients, R1, P1, leftCamera.imageSize, CV_32FC1, map1x, map1y);
        initUndistortRectifyMap(rightCamera.cameraMatrixRefined, rightCamera.distortionCoefficients, R2, P2, rightCamera.imageSize, CV_32FC1, map2x, map2y);
